...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // bouton de démarrage du tutoriel affiché sur l'écran d'accueil
  document.getElementById('start_tutorial').addEventListener('click', function() {
    ...
    startTutorial();                 // lance la séquence
  });
  ...
  // croix de fermeture quand le tutoriel est déjà en cours
  ...
  if (infoBtn) {
    // icône d'info permanente pour relancer le tutoriel
    ...
    setTimeout(function() {
      ...
      if (isMobile) {
        ...
        if (s.placement === 'left') {
          ...
          // Placement à droite de l'élément, centré verticalement
          left = rect.right + margin;
          top  = rect.top + (rect.height / 2) - (modalH / 2);
          if (left + modalW > vw - margin) left = rect.left - modalW - margin;
          if (top < margin)                top  = margin;
          if (top + modalH > vh - margin)  top  = vh - modalH - margin;
